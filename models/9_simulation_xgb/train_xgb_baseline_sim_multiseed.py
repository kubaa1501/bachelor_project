...
OUT_DIR = BASE_DIR / "models_new" / "xgb_baseline_sim_multiseed"
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    # save curves and plots across multiple seeds
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB Baseline (simulation)")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB Baseline Ranking (simulation)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    with open(OUT_DIR / "results_xgb_baseline_sim_multiseed.json", "w", encoding="utf-8") as f:
        json.dump(final_results, f, indent=2)
