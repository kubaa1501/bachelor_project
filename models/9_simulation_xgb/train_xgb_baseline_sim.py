...
OUT_DIR = BASE_DIR / "models_new" / "xgb_baseline_network_fixed_sim"
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB Baseline (new users sim)")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB Baseline Ranking (new users sim)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    final_results = {
        ...
        "dataset_type": "network_fixed_sim",
