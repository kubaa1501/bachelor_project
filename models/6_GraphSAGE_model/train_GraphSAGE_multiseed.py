...
#      multi-seed setup
# ---------------------------
# run final configuration with multiple seeds for mean ± std reporting
SEEDS = [42, 0, 1]

# ---------------------------
#      hyperparameters
# ---------------------------
# best hyperparameters selected during previous search
...
# commented out — other configurations tested during search
...
# embedding dimensions for categorical game features
CAT_EMB_DIMS = {
    "genres":    16,
    "developer": 64,
    "publisher": 64,
...
# move graph data to selected device once
data = data.to(DEVICE)
...
# ---------------------------
#   multi-seed train loop
# ---------------------------
etype = ("user", "plays", "game")

# store per-seed histories and final metrics
all_seeds_train_roc    = []
all_seeds_val_roc      = []
all_seeds_val_ndcg     = []
...
for seed in SEEDS:
    ...
    # initialize model for current seed
    model = LinkPredictor(
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        epoch_loader = LinkNeighborLoader(
            ...
            batch_size=batch_size, shuffle=False, # preserve group structure for BPR loss
        ...
        model.eval()
        with torch.no_grad():
            # approximate train ROC using a few batches (5)
            all_s, all_l = [], []
            ...
            # compute validation ranking metrics from validation scores
        ...
        # keep best checkpoint for current seed
    ...
    # save best model for current seed
    ...
    # reload best checkpoint for current seed
    ...
    # compute full graph embeddings once for final scoring
    with torch.no_grad():
    ...
    def score_df(df, batch_size=4096):
        # score valid user-game pairs using learned full-graph embeddings
        u_idx = torch.tensor(
    ...
    # evaluate current seed on validation and test sets
...
#           plots
# ---------------------------
# convert per-seed histories to arrays: (n_seeds, n_epochs)
...
#        save metrics
# ---------------------------
# save aggregate metrics, per-seed metrics and learning histories
metrics_dict = {
    "config": {
