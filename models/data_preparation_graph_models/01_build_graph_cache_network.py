...
# numeric user features for the network version
...
# categorical user features for the network version
USER_CATEGORICAL_FEATURES = ["country"]

# numeric game features for the network version
GAME_NUMERIC_FEATURES = [
    "game_total_playtime_minutes","user_count","release_date"
] + [f"game_emb_{i}" for i in range(32)]
# categorical game features for the network version
GAME_CATEGORICAL_FEATURES = ["genres","developer","publisher","platforms"]

# input files (network dataset)
files = {
    "train": "/home/jaga/data/sets/with_net_feat/train.csv",
    "val":   "/home/jaga/data/sets/with_net_feat/val.csv",
    "test":  "/home/jaga/data/sets/with_net_feat/test.csv"
}
...
t1 = time.time(); friends_df = pd.read_csv(friends_path);                             print(f"  friends:   {time.time()-t1:.1f}s, shape={friends_df.shape}")
...
# preprocess and store user node features
t1 = time.time()
...
# this graph structure is used in the network-enriched setup
t1 = time.time()
...
# save dataframes needed later for ranking evaluation
train_df.to_parquet(f"{cache_dir}/train_df.parquet")
val_df.to_parquet(f"{cache_dir}/val_df.parquet")
