...
OUT_DIR = BASE_DIR / "models_new" / "rf_network"
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    # train the best RF configuration on increasing fractions of train data
    # and measure:
    # - train ROC-AUC
    # - validation ROC-AUC
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — RF Network")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — RF Network Ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def main():
    # train, validate, test, and save RF network model
    ...
    # hyperparameters grid for model selection
    grid = [
        {
            "n_estimators": 80,
            "max_depth": 20,
