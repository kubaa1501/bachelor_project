...
# best hyperparameters selected earlier during search (code: train_rf_baseline.py)
BEST_PARAMS = {
    "n_estimators": 120,
    "max_depth": 30,
...
OUT_DIR = BASE_DIR / "models_new" / "rf_baseline_multiseed"
...
def make_learning_curve_multiseed(train_df, val_df, out_dir: Path):
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — RF Baseline (multi-seed)")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — RF Baseline Ranking (multi-seed)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def main():
    # train, validate, test, and save RF baseline model across multiple seeds
    ...
    # build learning curves for the final config
