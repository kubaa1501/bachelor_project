...
OUT_DIR = BASE_DIR / "models_new" / "xgb_usercount_emb0_only"
...
# numeric input features used by the model (only user_count and emb_0)
NUMERIC_FEATURES = [
    "user_count",
    "game_emb_0",
]
...
USECOLS = ID_COLS + [TARGET] + NUMERIC_FEATURES
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    for frac in LC_FRACTIONS:
        ...
        X_part = part[NUMERIC_FEATURES].copy()
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB user_count + emb0 only")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB user_count + emb0 only ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    grid = [
        {"n_estimators": 200, "max_depth": 3, "learning_rate": 0.05, "min_child_weight": 3, "subsample": 0.8, "colsample_bytree": 1.0, "reg_lambda": 1.0, "reg_alpha": 0.0},
        {"n_estimators": 300, "max_depth": 3, "learning_rate": 0.05, "min_child_weight": 3, "subsample": 0.8, "colsample_bytree": 1.0, "reg_lambda": 1.0, "reg_alpha": 0.0},
        {"n_estimators": 300, "max_depth": 4, "learning_rate": 0.05, "min_child_weight": 5, "subsample": 0.8, "colsample_bytree": 1.0, "reg_lambda": 1.0, "reg_alpha": 0.1},
        {"n_estimators": 400, "max_depth": 4, "learning_rate": 0.05, "min_child_weight": 5, "subsample": 1.0, "colsample_bytree": 1.0, "reg_lambda": 2.0, "reg_alpha": 0.1},
        {"n_estimators": 500, "max_depth": 5, "learning_rate": 0.03, "min_child_weight": 5, "subsample": 1.0, "colsample_bytree": 1.0, "reg_lambda": 2.0, "reg_alpha": 0.1},
    ]
