...
OUT_DIR = BASE_DIR / "models_new" / "xgb_baseline_without_user_count"
...
# numeric input features used by the model (no user_count)
NUMERIC_FEATURES = [
    "total_games_owned",
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB Baseline without user count")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB Baseline without user count Ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
