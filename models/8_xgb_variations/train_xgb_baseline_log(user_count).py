...
OUT_DIR = BASE_DIR / "models_new" / "xgb_baseline_log_user_count"
...
# user_count is removed from the standard numeric feature list
# and processed in a dedicated log-transform pipeline
LOG_USER_COUNT_FEATURE = ["user_count"]
...
USECOLS = ID_COLS + [TARGET] + LOG_USER_COUNT_FEATURE + NUMERIC_FEATURES + CATEGORICAL_FEATURES
...
def build_pipeline(params):
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            ...
            ("log_user_count", log_user_count_transformer, LOG_USER_COUNT_FEATURE),
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    rows = []
    X_val = val_df[LOG_USER_COUNT_FEATURE + NUMERIC_FEATURES + CATEGORICAL_FEATURES].copy()
    ...
    for frac in LC_FRACTIONS:
        ...
        X_part = part[LOG_USER_COUNT_FEATURE + NUMERIC_FEATURES + CATEGORICAL_FEATURES].copy()
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB Baseline log(user_count)")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB Baseline log(user_count) Ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    feature_cols = LOG_USER_COUNT_FEATURE + NUMERIC_FEATURES + CATEGORICAL_FEATURES
