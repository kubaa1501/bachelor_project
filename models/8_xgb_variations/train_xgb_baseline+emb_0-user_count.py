...
OUT_DIR = BASE_DIR / "models_new" / "xgb_emb0_without_user_count"
...
# numeric input features used by the model (no user_count, +emb_0)
NUMERIC_FEATURES = [
    "total_games_owned",
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB emb0 without user_count")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB emb0 without user_count Ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def main():
    ...
    final_results = {
        "model_type": "xgb",
        "dataset_type": "emb0 without user_count",
        "seed": SEED,
