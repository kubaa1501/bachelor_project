...
USECOLS = ID_COLS + [TARGET] + [c for c in NUMERIC_FEATURES if c != FAKE_EMB_COL] + CATEGORICAL_FEATURES
...
def build_fake_feature_with_target_corr(
    x: pd.Series,
    rho: float = 0.97,
    seed: int = 42,
    ...
    fake = pd.Series(fake_z, index=x.index, name=FAKE_EMB_COL)

    if scale_to_original_std:
        x_num = pd.to_numeric(x, errors="coerce")
        x_std = x_num.std(ddof=0)
...
def add_fake_embedding_feature(df: pd.DataFrame, split_name: str) -> pd.DataFrame:
    """
    Add fake embedding feature derived from user_count with target correlation ~0.97.
    Uses a different deterministic seed per split.
    """
    df = df.copy()
    ...
    seed = SEED + split_offsets.get(split_name.lower(), 999)
    ...
    if valid_mask.sum() < 3:
        raise ValueError(
            f"Need at least 3 non-null rows in '{FAKE_EMB_SOURCE_COL}' to build fake embedding."
        )
    ...
    fake_valid = build_fake_feature_with_target_corr(
        ...
        seed=seed,
        scale_to_original_std=True,
    )

    df[FAKE_EMB_COL] = np.nan
    df.loc[valid_mask, FAKE_EMB_COL] = fake_valid

    # fill any remaining NA in the fake feature with median, so downstream pipeline stays simple
    median_fake = df[FAKE_EMB_COL].median()
    if pd.isna(median_fake):
        median_fake = 0.0
...
def read_split(path: Path, max_rows: int = 0) -> pd.DataFrame:
    ...
    split_name = path.stem.lower()
    df = add_fake_embedding_feature(df, split_name=split_name)
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB Baseline + Fake Embedding")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB Baseline + Fake Embedding Ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def main():
    # train, validate, test, and save XGB baseline model
