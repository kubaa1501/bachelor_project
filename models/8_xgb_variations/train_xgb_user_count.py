...
OUT_DIR = BASE_DIR / "models_new" / "xgb_user_count"
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB user_count")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB user_count ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
