...
OUT_DIR = BASE_DIR / "models_new" / "xgb_one_embedding"
...
# numeric input features used by the model (baseline + emb__0)
NUMERIC_FEATURES = [
    "total_games_owned",
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB One Embedding")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB One Embedding Ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
