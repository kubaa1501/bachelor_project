...
OUT_DIR = BASE_DIR / "models_new" / "xgb_embeddings_only"
...
# numeric input features used by the model (only embeddings)
NUMERIC_FEATURES = [
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB Embeddings Only")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB Embeddings Only Ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
