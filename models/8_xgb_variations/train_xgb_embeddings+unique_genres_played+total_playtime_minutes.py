...
OUT_DIR = BASE_DIR / "models_new" / "xgb_embeddings_and_basic_info"
...
# numeric input features used by the model (embeddings + unique_genres_playes + total_playtime_minutes)
NUMERIC_FEATURES = [
...
def make_learning_curve(best_params, train_df, val_df, out_dir: Path):
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — XGB Embeddings and Basic Info")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — XGB Embeddings and Basic Info Ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
