...
OUT_DIR = BASE_DIR / "models_new" / "lr_network"
...
# preprocessor is fit only on a sample of the training to reduce memory 
...
def iter_split(path: Path, max_rows: int = 0, chunksize: int = 200_000):
    ...
    # this is used for large-scale training so we do not need to load the entire training split into memory at once
    rows_read = 0
...
def sparse_to_csr_int32(X):
    # ensure that a sparse matrix is stored in CSR format with int32 indices (technical safeguard for large sparse feature matrices)
...
def build_model(C: float, class_weight):
...
def fit_preprocessor(train_csv: Path, max_train_rows: int = 0):
    #fit preprocessor on a sample of the training
    print("\n" + "=" * 80)
    ...
    # transform a small preview to inspection
...
def transform_features(preprocessor, df: pd.DataFrame):
    # apply the fitted preprocessor and convertion
...
def fit_model_chunked(preprocessor, train_csv: Path, C: float, class_weight, max_train_rows: int = 0):
    ...
    for epoch in range(1, EPOCHS + 1):
        print("\n" + "=" * 80)
        print(f"Training epoch {epoch}/{EPOCHS} for params C={C}, class_weight={class_weight}")
        ...
        for chunk_idx, chunk in enumerate(iter_split(train_csv, max_rows=max_train_rows, chunksize=CHUNKSIZE), start=1):
            ...
            print(
                f"[TRAIN] epoch={epoch} chunk={chunk_idx} rows={len(chunk)} "
                f"fit_seconds={dt:.2f} coef_norm={chunk_log['coef_norm']:.6f}"
            )

        epoch_dt = time.time() - epoch_start
        print(
            f"[TRAIN] epoch {epoch} done: chunks={epoch_chunks}, rows={epoch_rows}, "
            f"elapsed={epoch_dt:.2f}s"
        )

    return model, pd.DataFrame(fit_log)

# transform features and return predicted probabilities for positive 
...
def make_learning_curve(best_params, preprocessor, train_df, val_df, out_dir: Path):
    # build a learning curve for the best parameter setting
    rows = []
    ...
    plt.ylabel("ROC-AUC")
    plt.title("Learning Curve — LR Network")
    ...
    plt.xlabel("Training rows")
    plt.ylabel("Score")
    plt.title("Learning Curve — LR Network Ranking")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def main():
    # train, val, test and save the LR network model
    ...
    best_val_row = None
    best_fit_log = None
    # train one chkunked model per hyperparameter setting
    for i, params in enumerate(grid, start=1):
        ...
        fit_seconds = time.time() - t0
        # evaluate on validation and select best model by NDCG@10
        ...
        fit_log_df.to_csv(OUT_DIR / f"fit_log_trial_{i}.csv", index=False)
        ...
        if val_metrics["NDCG@10"] > best_val_ndcg10:
            ...
            best_params = params
            best_val_row = row
            best_fit_log = fit_log_df
    # save val trail summary 
    pd.DataFrame(all_trials).to_csv(OUT_DIR / "val_trials.csv", index=False)

    print("\nBest params:")
    print(best_params)
    print("Best VAL NDCG@10:", best_val_ndcg10)
    #  load and evaluate on test split 
    test_df = read_split(TEST_CSV, max_rows=MAX_TEST_ROWS)
    test_auc, test_metrics, per_user_metrics, test_score_df = evaluate_dataset(preprocessor, best_model, test_df, "test")
    # save predictions and per-user ranking metrics
    ...
    # save preprocessor, model and best fit log 
    joblib.dump(preprocessor, OUT_DIR / "preprocessor.joblib")
    joblib.dump(best_model, OUT_DIR / "best_model.joblib")
    if best_fit_log is not None:
        best_fit_log.to_csv(OUT_DIR / "best_fit_log.csv", index=False)
    # final summary .json
    final_results = {
        "model_type": "lr",
    ...
    make_learning_curve(best_params, preprocessor, train_df_lc, val_df_lc, OUT_DIR)
